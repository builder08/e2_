...
from enigma import eEPGCache, eServiceEvent, eServiceReference, iServiceInformation
		...
		if event:
			match self.token:
				case self.CRID_EPISODE:
					result = trimText(getCRID(event, eServiceEvent.EPISODE_MATCH))
				case self.CRID_RECOMMENDATION:
					result = trimText(getCRID(event, eServiceEvent.RECOMMENDATION_MATCH))
				case self.CRID_SERIES:
					result = trimText(getCRID(event, eServiceEvent.SERIES_MATCH))
